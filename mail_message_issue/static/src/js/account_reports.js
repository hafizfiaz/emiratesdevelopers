odoo.define('accounting_reports_filters.account_report', function (require) {
'use strict';
...
//            domain: [['parent_id', '=', this.fields.asset_project_id.value]],
            value: fieldInfo.value,
        }], options).then(function (recordID) {
            self.widgets[fieldName] = new RelationalFields.FieldMany2ManyTags(self,
                fieldName,
...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
        'value_changed': function(ev) {
             ...
             self.report_options.asset_project_id = ev.data.asset_project_id;
             self.report_options.property_id = ev.data.property_id;
             self.report_options.nationality_id = ev.data.nationality_id;
             self.report_options.ex_nationality_id = ev.data.ex_nationality_id;
             self.report_options.related_spa_id = ev.data.related_spa_id;
             self.report_options.sale_type = ev.data.sale_type;
             self.report_options.receivable_status_id = ev.data.receivable_status_id;
             self.report_options.ex_receivable_status_id = ev.data.ex_receivable_status_id;
             self.report_options.type_id = ev.data.type_id;
             self.report_options.account_id = ev.data.account_id;
    ...
    render_searchview_buttons: function() {
        ...
        if (this.report_options.partner) {
            if (!this.M2MFilters) {
                ...
                if ('asset_project_id' in this.report_options) {
                    fields['asset_project_id'] = {
                        label: _t('Projects'),
                        modelName: 'account.asset.asset',
                        value: this.report_options.asset_project_id.map(Number),
                    };
                    fields['property_id'] = {
                        label: _t('Property'),
                        modelName: 'account.asset.asset',
                        domain: [['parent', 'in',this.report_options.asset_project_id ]],
                        value: this.report_options.property_id.map(Number),
                    };
                }
                if ('property_id' in this.report_options) {
                    fields['property_id'] = {
                        label: _t('Property'),
                        modelName: 'account.asset.asset',
                        value: this.report_options.property_id.map(Number),
                ...
                if ('nationality_id' in this.report_options) {
                    fields['nationality_id'] = {
                        label: _t('Nationality'),
                        modelName: 'res.country',
                        value: this.report_options.nationality_id.map(Number),
                    };
                }
                if ('ex_nationality_id' in this.report_options) {
                    fields['ex_nationality_id'] = {
                        label: _t('Nationality Excluded'),
                        modelName: 'res.country',
                        value: this.report_options.ex_nationality_id.map(Number),
                    };
                }
                if ('related_spa_id' in this.report_options) {
                    fields['related_spa_id'] = {
                        label: _t('Related SPA'),
                        modelName: 'sale.order',
                        value: this.report_options.related_spa_id.map(Number),
                    };
                }
                if ('receivable_status_id' in this.report_options) {
                    fields['receivable_status_id'] = {
                        label: _t('Receivable Status'),
                        modelName: 'receivable.status',
                        value: this.report_options.receivable_status_id.map(Number),
                    };
                }
                if ('ex_receivable_status_id' in this.report_options) {
                    fields['ex_receivable_status_id'] = {
                        label: _t('Receivable Status Excluded'),
                        modelName: 'receivable.status',
                        value: this.report_options.receivable_status_id.map(Number),
                    };
                }

                if ('type_id' in this.report_options) {
                    fields['type_id'] = {
                        label: _t('Property Type'),
                        modelName: 'property.type',
                        value: this.report_options.type_id.map(Number),
                    };
                }
