...
class MomentSecurityReport(models.AbstractModel):
    _name = 'report.pdc_movement.pdc_security_movement_report'
    ...
    def get_result(self, start_date, end_date):
        ...
        collected_cheques = self.env['account.payment'].search([('collection_type_id.name','in',['Security Cheques']),('payment_type','=','inbound'),('date','<',start_date),('state','=','collected'),('chk','=',True)])
        posted_cheques = self.env['account.payment'].search([('collection_type_id.name','in',['Security Cheques']),('payment_type','=','inbound'),('date','<',start_date),('state','not in',['draft','cancelled','collected']),('paid_date','>=',start_date),('paid_date','<=',end_date),('chk','=',True)])
        total_collected = 0.0
        ...
        cheques_deposited = self.env['account.payment'].search([('collection_type_id.name','in',['Security Cheques']),('date','>=',start_date),('date','<=',end_date),('payment_type','=','inbound'),
                                                              ('state','not in',['draft','cancelled']),('chk','=',True)])
        total = 0.0
        ...
        cheques_cleared = self.env['account.payment'].search([('collection_type_id.name','in',['Security Cheques']),('date','<',end_date),('payment_type','=','inbound'),
                                                              ('state_update_date','>=',start_date),
                                                              ('state_update_date','<=',end_date),
                                                              ('chk','=',True)])
        total = 0.0
        ...
        cheques_stale =  self.env['account.payment'].search([('collection_type_id.name','in',['Security Cheques']),('date','<',end_date),('payment_type','=','inbound'),
