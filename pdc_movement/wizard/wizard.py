...
class PdcMovementReportWizard(models.TransientModel):
    _name = "pdc.movement.report.wizard"
    ...
    def check_report(self):
        ...
        return {
            ...
            'data': {'model': 'pdc.movement.report.wizard',
    ...
    def get_xlsx(self, options, response=None):
        ...
        data['ids'] = []
        env_obj = vals.env['report.pdc_movement.pdc_movement_report']
        ...
        sheet.merge_range(3, 0, 1, 3, "PDCs Movement", format1)
        ...
        row += 1
        sheet.write(row, 0, "Additional Cheques during the period-Regular", format3)
        ...
        row += 1
        sheet.write(row, 0, "Additional Cheques during the period-Rental", format3)
        sheet.write(row, 1, result['cheques rental'][1], format7)
        sheet.write(row, 2, result['cheques rental'][0], format7)
        total_count += result['cheques rental'][1]
        total += result['cheques rental'][0]
        row += 1
        sheet.write(row, 0, "Additional Cheques during the period-EOI", format3)
        ...
        row += 1
        sheet.write(row, 0, "Cheques Deposited- In Clearance", format3)
        ...
        row += 1
        sheet.write(row, 0, "Cheques Stale/Withdraw/Replaced/Settled during the period", format3)
        ...
        row += 1
        sheet.write(row, 0, "Cheques Bounced during the period", format3)
        ...
        sheet2 = workbook.add_worksheet(str('Rental Cheques'))
        ...
        sheet2.set_column('A:Y', 15)
        row1 = 2
        for rec in result['cheques rental view']:
            sheet2.write(row1, 1, rec['create_date'].strftime("%Y/%m/%d"), format7)
            sheet2.write(row1, 2, rec['date'].strftime("%Y/%m/%d"), format7)
            sheet2.write(row1, 3, rec['name'], format7)
            sheet2.write(row1, 4, rec['journal_name'], format7)
        ...
        sheet2.set_column('A:Y', 15)
        row1 = 2
        for rec in result['cheques bounced view']:
            sheet2.write(row1, 1, rec['create_date'].strftime("%Y/%m/%d"), format7)
            sheet2.write(row1, 2, rec['date'].strftime("%Y/%m/%d"), format7)
            sheet2.write(row1, 3, rec['name'], format7)
            sheet2.write(row1, 4, rec['journal_name'], format7)
