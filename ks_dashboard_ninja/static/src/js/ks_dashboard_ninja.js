...
//                        "ks_board_active_user_ids": [[6, false, [odoo.session_info.uid]]],
       ...
       var self = this;
        ...
        renderListViewData: function(item) {
            ...
            if (list_view_data) {
                ...
                for (var i = 0; i < list_view_data.data_rows.length; i++) {
                    for (var j = 0; j < list_view_data.data_rows[0]["data"].length; j++) {
                        if (typeof(list_view_data.data_rows[i].data[j]) === "number" || list_view_data.data_rows[i].data[j]) {
                            if (typeof(list_view_data.data_rows[i].data[j]) === "number" || !isNaN(parseInt(list_view_data.data_rows[i].data[j].replace(/,/g, '')))) {
                                total[j] = 0.0
                            }
                        }
                    }
            ...
            if (list_view_data) {
                for (var i = 0; i < list_view_data.data_rows.length; i++) {
                    for (var j = 0; j < list_view_data.data_rows[0]["data"].length; j++) {
                        if (typeof(list_view_data.data_rows[i].data[j]) === "number" || list_view_data.data_rows[i].data[j]) {
                            if (typeof(list_view_data.data_rows[i].data[j]) === "number" || !isNaN(parseInt(list_view_data.data_rows[i].data[j].replace(/,/g, '')))) {
                                total[j] += parseFloat(list_view_data.data_rows[i].data[j].replace(/,/g, ''))
                                list_view_data.data_rows[i].data[j] = field_utils.format.float(parseFloat(list_view_data.data_rows[i].data[j].replace(/,/g, '')), Float64Array)
                            }
                        } else {
                            list_view_data.data_rows[i].data[j] = "";
                        }
            ...
            for (var key in total) {
                if (total.hasOwnProperty(key)) {
                    arr.push( [total[key].toLocaleString("en-US")]);
                }
            }
            debugger;
            var $ksItemContainer = $(QWeb.render('ks_list_view_table', {
                ...
                add_total: list_view_data.ks_addtotal,
                total: arr,
                isDrill: self.ks_dashboard_data.ks_item_data[item_id]['isDrill']
            }));
            self.list_container = $ksItemContainer;
            if (list_view_data){
                var $ksitemBody = self.ksListViewBody(list_view_data,item_id,arr)
        ...
        ksListViewBody: function(list_view_data, item_id, total) {
            var self = this;
            debugger;
            var itemid = item_id
            var total = total
