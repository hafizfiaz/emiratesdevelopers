odoo.define('property_website.property_website_rpc', function(require) {
    ...
    /*if (window.location.pathname == '/'){
            debugger;
            window.location.pathname = '/all_asset_lease_golf'
//                window.history.pushState("All Past Sales", "All Past Sales", "/all_past_sale"+"?"+"page="+view['page']);
//                allassetpastsalepage(view)
            }*/
//        if (window.location.pathname == '/all_asset_lease_golf'){
//                    $('#top_menu').children().hide();
//                    $('#top_menu').children()[4].style.display='block';
//
//                    $('#top_menu_collapse_clone').children().children().hide();
////                    $('#top_menu_collapse_clone').children().children()[4].style.display='block';
//
//                }
        ...
        $('.wizard').click(function() {
            event.target.closest('.rent-content').getAttribute("data-lease_id")
            $(":submit").show();
            ...
            debugger;
                ...
                }else if (event.target.getAttribute("class") === "btn"){
                    for(var i = 0; i < document.getElementsByClassName("modal").length; i++){
                        document.getElementsByClassName("modal")[i].style.display = 'none'
                        }
                }else if (event.target.getAttribute("class") === "payments"){
                    event.target.childNodes[1].style.display = "block";
                }else{
//                    document.getElementById('modal').style.display = 'none'
                    for(var i = 0; i < document.getElementsByClassName("modal").length; i++){
                        if (document.getElementsByClassName("modal")[i].getAttribute("data-lease_id")===event.target.closest('.rent-content').getAttribute("data-lease_id")){
                        document.getElementsByClassName("modal")[i].style.display = 'block';
                        }
//                        document.getElementsByClassName("modal")[i].style.visibility = "hidden"; // or
//                        document.getElementsByClassName("modal")[i].style.display = "none"; // depending on what you're doing
                    }
                }
        });
        ...
        $('#reset_templates_button').click(function() {
        ...
        if (window.history && window.history.pushState) {
            $(window).on('popstate', function() {
                if (window.location.pathname == '/' || window.location.pathname == '/page/homepage'){
                    ...
                    document.title = 'Homepage';
    //                window.history.pushState("Homepage", "Homepage", "/");
                    odoo.jsonRpc("allassetlease_display", 'call', {}).then(function(data) {
                        $('main').fadeOut("slow", function() {
                        debugger;
                            $('main').html(data).fadeIn("slow");
                            $('.navbar-default a').parent().removeClass('active');
...
//                console.log('all_property_cookies_ids---------->', all_property_cookies_ids)
    ...
    //    code for render lease page
        function allassetleasepage_golf(value){
            document.title = 'All Lease';
            if (window.location.pathname != '/all_asset_lease_golf'){
                window.history.pushState("All Lease", "All Lease", "/all_asset_lease_golf");
            }
//            console.log('window.location.pathname-------------->', window.location.pathname)
            if (value === undefined) {
                value = {}
            ...
            var page = ca[1]
            value['page'] = page
            odoo.jsonRpc("/allassetlease_display_golf", 'call', value).then(function(data) {
                $('main').fadeOut("slow", function() {
//                    console.log('-----------CALLkp---')
                    $('main').html(data).fadeIn(0);
                    displayTotalCookieProducts()
                    var property_type = $("#property_type").val();
                    window.sessionStorage.setItem('display_type', property_type);
                    $('.navbar-default a').parent().removeClass('active');
        ...
        $(document).on('click', '#view_all_asset_lease_golf', function(e){
            allassetleasepage_golf()
        });

        $(document).on('click', '#js_check_product', function(e){
            print_golf()
        });
    ...
    //        $('#check_property:checked').each(function(){
        ...
        $(document).on('click', '#search_property_filter', function(e){
            debugger;
            var $this = $(this);
//            var form_filter_class = $(this).parent().parent().parent().parent().parent('form.form_filter_rent_unit');
            var total_selected_property_type_ids = [];
            debugger;
            url = event.target.formAction;
            property_url = url.substring(url.lastIndexOf('/') + 1);
            id = url.substring(url.lastIndexOf('=') + 1);
            window.history.pushState("All Lease", "All Lease", "/".concat(property_url));
            odoo.jsonRpc("/allassetlease_display_golf", 'call',{
                'number' : $('#number').val(),
                'id' : parseInt(id),
                'unittype' : $('#unittypelist').val(),
                'viewlist' : $('#viewlist').val(),
                'status' : $('#statelist').val(),
                'total_selected_property_type_ids': total_selected_property_type_ids,
            }).then(function(data) {
            $('.recent-viewed-content').fadeOut(50, function() {
                debugger;
                $('.recent-viewed-content').html(data).fadeIn(50);
                displayTotalCookieProducts()
                var property_type = $("#property_type").val();
                window.sessionStorage.setItem('display_type', property_type);
                $('.navbar-default a').parent().removeClass('active');
...
//            var form_filter_class = $(this).parent().parent().parent().parent().parent('form.form_filter_rent_unit');
            ...
            $('.recent-viewed-content').fadeOut(50, function() {
                ...
                $('.navbar-default a[href="/rent"]').parent().addClass('active');
                debugger;
                $("recent-viewed-content").load("This is my Page");
//                homepage_search();
            });
//            $(window).scrollTop(0);
            });
        });*/
//        $(document).on('change', '#unittypelist', function(e){
//            debugger;
//            var $this = $(this);
////            var form_filter_class = $(this).parent().parent().parent().parent().parent('form.form_filter_rent_unit');
//            var total_selected_property_type_ids = [];
...
//            window.history.pushState("All Lease", "All Lease", "/all_asset_lease_golf");
//            odoo.jsonRpc("/allassetlease_display_golf", 'call',{
//                'unittype' : parseInt(jQuery(this).val()),
//                'total_selected_property_type_ids': total_selected_property_type_ids,
//            }).then(function(data) {
//            $('main').fadeOut("slow", function() {
//                debugger;
...
//        });
        ...
        $(document).on('click', '#unitbtn', function(e){
            var $this = $(this);
            var form_filter_class = $(this).parent().parent().parent().parent().parent('form.form_filter_rent_unit');
            var total_selected_property_type_ids = [];
            debugger;
            window.history.pushState("All Lease", "All Lease", "/all_asset_lease_golf");
            odoo.jsonRpc("/allassetlease_display_golf", 'call',{
                'unittype' : form_filter_class.find('input.unit_type').val(),
                'total_selected_property_type_ids': total_selected_property_type_ids,
            }).then(function(data) {
            $('main').fadeOut("slow", function() {
                debugger;
        ...
        function search_properties_lease_sale_page_golf(value){
            var property_type = $("#property_type").val();
            if (value === undefined) {
                value = {}
            }
...
//        var btn_one = document.getElementsByClassName("#myBtn");
//
...
//            debugger;
//            if (this.getAttribute("class") === "myBtn"){
...
//            }
            ...
            window.onclick = function(event) {
                event.target.getAttribute("data-lease_id")
                if (event.target.getAttribute("class") === "close"){
                    for(var i = 0; i < document.getElementsByClassName("modal").length; i++){
                        document.getElementsByClassName("modal")[i].style.display = 'none'
                        }
...
//                    document.getElementById('modal').style.display = 'none'
                    for(var i = 0; i < document.getElementsByClassName("modal").length; i++){
                        if (document.getElementsByClassName("modal")[i].getAttribute("data-lease_id")===event.target.getAttribute("data-lease_id")){
                        document.getElementsByClassName("modal")[i].style.display = 'block';
                        }
//                        document.getElementsByClassName("modal")[i].style.visibility = "hidden"; // or
//                        document.getElementsByClassName("modal")[i].style.display = "none"; // depending on what you're doing
...
//        $(document).on('click', '#hidden_box_btn', function(){
//            debugger;
//            $('#hidden_box').modal('show');
//        });
    ...
    //        }else if($(this).text() == 'Prev'){
            ...
            if (window.location.pathname == '/all_asset_lease_golf'){
                window.history.pushState("All Lease", "All Lease", "/all_asset_lease_golf"+"?"+"page="+view['page']);
                search_properties_lease_sale_page_golf(view)
            }
            if (window.location.pathname == '/all_asset_sale'){
