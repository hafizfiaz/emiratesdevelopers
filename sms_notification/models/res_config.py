...
class ResConfigSettings(models.TransientModel):
    ...
    def _check_rml(self):
        result = self.env['ir.module.module'].search(
            [('name', '=', 'bi_sms_client_generic')])
        if result:
            if self:
                self.module_rml_gateway = True
            return True
        else:
            return False
    ...
    module_rml_gateway = fields.Boolean(
        string='Install RML SMS Gateway', help='It will Install Rml sms gateway automatically.')
    is_rml_in_addon = fields.Boolean(default=_check_rml)
    ...
    module_messagebird_gateway = fields.Boolean(
        string='Install Messagbird SMS Gateway', help='It will Install Messagebird sms gateway automatically.', default=False)
