...
class AccountVoucherCollectionReport(models.AbstractModel):
    _name = 'report.account_voucher_collection.report_multi_template'

    @api.model
    def _get_report_values(self, docids, data=None):
        docs = self.env['account.voucher.collection'].search([('id', 'in', docids)])
        flag = self.env['res.users'].has_group('account.group_account_user')
        if not flag:
            if docs.state not in ['collected']:
